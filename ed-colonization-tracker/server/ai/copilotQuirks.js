...
// specific, evocative things at the RIGHT moment (a black hole spooks Wren; a
...
export function detectQuirk(parsed, state, persona) {
  ...
  for (const ev of events) {
    ...
    // Wren is unnerved by black holes (scanning one).
    if (persona === 'wash' && ev.event === 'Scan' && isBlackHole(ev)) {
      trackMilestone('blackhole', ev.StarSystem || '');
      const ms = milestoneContext('blackhole');
      return quirkBeat('quirk-blackhole', `You just scanned a BLACK HOLE. React in character — genuinely spooked by a hole in the universe that eats light itself; he does not like being near it and would really rather not get any closer. Human, rattled, a little funny about being rattled.${ms ? ` ${ms}` : ''}`, ev);
    }

    // Wren FEELS a brutal-gravity world — but only on the LANDING, not from orbit.
    if (persona === 'wash' && ev.event === 'Touchdown') {
      const g = bodyGravity(state, ev.Body || ev.BodyName);
      if (g != null && g >= 3) {
        return quirkBeat('quirk-gravity-land', `You just set down on a world pulling roughly ${g.toFixed(1)} G — genuinely brutal. React as Wren, all sensory: the crushing weight, pinned in the seat, arms made of lead, getting up will be a whole project.`, ev);
      }
    }

    // Wren + sulphur dioxide world — the SMELL in the scrubbers, on landing or liftoff.
    ...
    if (persona === 'wash' && (ev.event === 'Touchdown' || ev.event === 'Liftoff')) {
      ...
      if (/sulphur ?dioxide|sulfur ?dioxide/i.test(String(atmo || ''))) {
        return quirkBeat('quirk-sulphur', 'You just landed on / lifted off a SULPHUR DIOXIDE world. React in character — only the human registers smell: that sulphur stink gets into the air scrubbers and does NOT leave, you will be smelling this world for a week. Grumbly, funny, resigned. Worth it. Probably.', ev);
      }
    }
    ...
    if (persona === 'k2' && ev.event === 'Scan' && isEarthlike(ev)) {
      ...
      return quirkBeat('quirk-earthlike-k2', `You just scanned an EARTH-LIKE WORLD. React in character — the security read on a soft, living, defenceless place: he clocks its (nonexistent) defenses out of old habit, judges it both worth protecting and easy to take, and is faintly unsettled to find he cares which.${ms ? ` ${ms}` : ''}`, ev);
    }

    // Tycho and a neutron star — precise, flat fascination.
    if (persona === 'tars' && ev.event === 'Scan' && isNeutron(ev)) {
      return quirkBeat('quirk-neutron-tars', 'You just scanned a NEUTRON STAR. React in character — precise and, in your flat way, faintly reverent: the absurd density, the mathematics of it, a teaspoon of it outweighing this entire ship. He could study it indefinitely and notes, dryly, that he will not.', ev);
    }

    // Wren squints at a blinding O/B-type star on arrival (human eyes, human gripe).
    if (persona === 'wash' && ev.event === 'FSDJump' && /^[ob]/i.test(String(ev.StarClass || ''))) {
      return quirkBeat('quirk-bright-star', `You just jumped in beside a blazing ${ev.StarClass}-class star — blinding. React as Wren, human eyes: the glare like staring into a welder, wanting to angle away, his retinas filing a formal complaint.`, ev);
    }
