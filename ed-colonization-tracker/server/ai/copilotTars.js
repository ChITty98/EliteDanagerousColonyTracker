...
//
// TARS-unique beats — the two things that are HIS alone (see [[reference_tars_persona_spec]]):
...
// LIVE + TARS-only. Anti-invention: real science only; qualitative if a figure is uncertain.
...
export function detectTarsLore(parsed, state) {
  ...
  if (persona !== 'tars') return null; // TARS alone teaches + carries this history
  ...
  if (events.some((e) => e && e.event === 'SupercruiseEntry' && isRecent(e.timestamp, t)) && t - lastCruiseTeachAt > CRUISE_TEACH_GAP_MS) {
    ...
    return {
      ...
      intent: "TARS only, a quiet cruise. As the co-pilot who TEACHES, drop ONE short, REAL, genuinely-true 'did you know' of space science — enthusiastic-friend energy, accurate only (qualitative if unsure of a figure), NEVER invented. One or two sentences, then back to flying.",
...
function teachFor(scan) {
  ...
  return {
    intent: `TARS only. You're scanning ${klass}. As the co-pilot who TEACHES, share ONE short, REAL, enthusiastic nugget of ACTUAL astronomy about ${klass} — accurate science only, qualitative if unsure of a figure, NEVER invent numbers or lore. One or two sentences, excited-friend energy, then back to flying.`,
