...
const serverLines = [
  ...
  'var BACKUP_DIR = pathMod.join(appDir, "backups");',
  'try { fs.mkdirSync(BACKUP_DIR, { recursive: true }); } catch(e) {}',
  'try {',
  '  var stateSize = fs.statSync(STATE_FILE).size;',
  '  if (stateSize > 100) {',
  '    var ts = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);',
  '    var backupPath = pathMod.join(BACKUP_DIR, "colony-data." + ts + ".json");',
  '    fs.copyFileSync(STATE_FILE, backupPath);',
  '    console.log("[Backup] Created " + backupPath + " (" + (stateSize / 1024).toFixed(0) + "KB)");',
  '    var backups = fs.readdirSync(BACKUP_DIR).filter(function(f) { return f.startsWith("colony-data."); }).sort();',
  '    while (backups.length > 5) { try { fs.unlinkSync(pathMod.join(BACKUP_DIR, backups.shift())); } catch(e) {} }',
  '  }',
  ...
  '      try {',
  '        var newJson = JSON.stringify(pendingState);',
  '        try {',
  '          var existingSize = fs.statSync(STATE_FILE).size;',
  ...
  '            return;',
  '          }',
  '        } catch(e) { /* file does not exist yet, ok to write */ }',
  ...
  '  }',
  '',
  '  // Exploration API: GET /api/exploration/:addr',
  '  var exploMatch = pathname.match(/^\\/api\\/exploration\\/(\\d+)$/);',
  '  if (exploMatch && req.method === "GET") {',
  '    var addr = exploMatch[1];',
  '    var stData = readStateFile();',
  '    var exploCache = stData.journalExplorationCache || {};',
  '    var exploSystem = exploCache[addr] || null;',
  '    res.writeHead(200, { "Content-Type": "application/json" });',
  '    res.end(JSON.stringify(exploSystem));',
  '    return;',
  '  }',
  '',
  ...
  '        var sourceIp = (req.headers["x-forwarded-for"] || req.socket.remoteAddress || "").toString();',
