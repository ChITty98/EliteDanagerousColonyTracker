...
const serverLines = [
  ...
  '    var isOpShape = val && typeof val === "object" && !Array.isArray(val) && ("__upsert" in val || "__remove" in val || "__add" in val);',
  '    if (!isOpShape) { out[key] = val; continue; }',
  '    var hasUpsert = val.__upsert && typeof val.__upsert === "object";',
  '    var hasIdKey = typeof val.__idKey === "string";',
  '    var removeList = Array.isArray(val.__remove) ? val.__remove : [];',
  '    var addList = Array.isArray(val.__add) ? val.__add : [];',
  '    if (hasIdKey) {',
  '      var idKey = val.__idKey;',
  '      var curArr = Array.isArray(existing[key]) ? existing[key] : [];',
  '      var map = {};',
  '      for (var i = 0; i < curArr.length; i++) { var it = curArr[i]; if (it && it[idKey] != null) map[String(it[idKey])] = it; }',
  '      if (hasUpsert) { for (var id in val.__upsert) { if (Object.prototype.hasOwnProperty.call(val.__upsert, id)) map[id] = val.__upsert[id]; } }',
  '      for (var r = 0; r < removeList.length; r++) delete map[String(removeList[r])];',
  '      out[key] = Object.values(map);',
  '    } else if (addList.length > 0 || (removeList.length > 0 && !hasUpsert)) {',
  '      var curArr2 = Array.isArray(existing[key]) ? existing[key] : [];',
  '      var removeSet = {}; for (var rr = 0; rr < removeList.length; rr++) removeSet[removeList[rr]] = true;',
  '      var next = []; for (var n = 0; n < curArr2.length; n++) { if (!removeSet[curArr2[n]]) next.push(curArr2[n]); }',
  '      for (var a = 0; a < addList.length; a++) { if (next.indexOf(addList[a]) < 0) next.push(addList[a]); }',
  '      out[key] = next;',
  '    } else {',
  '      var cur = (existing[key] && typeof existing[key] === "object" && !Array.isArray(existing[key])) ? existing[key] : {};',
  '      var merged = Object.assign({}, cur);',
  '      if (hasUpsert) { for (var k in val.__upsert) { if (Object.prototype.hasOwnProperty.call(val.__upsert, k)) merged[k] = val.__upsert[k]; } }',
  '      for (var rk = 0; rk < removeList.length; rk++) delete merged[removeList[rk]];',
  '      out[key] = merged;',
  '    }',
  '  }',
  '  return out;',
  ...
  '  }',
  '',
  '  // Token API: GET /api/network-url (localhost only)',
  '  if (pathname === "/api/network-url" && req.method === "GET") {',
  '    if (!isLocalhost(req)) { res.writeHead(403); res.end("{}"); return; }',
  '    var hostname = os.hostname();',
  '    var networkUrl = "http://" + hostname + ":" + PORT + "?token=" + APP_TOKEN;',
  '    res.writeHead(200, { "Content-Type": "application/json" });',
  '    res.end(JSON.stringify({ url: networkUrl, token: APP_TOKEN, hostname: hostname, port: PORT }));',
  '    return;',
  '  }',
  '',
  ...
  '  }',
  '',
  '  // State API: PATCH /api/state — sparse diff merge',
  ...
  '        var merged = mergeStatePatch(readStateFile(), incoming);',
  ...
  '  }',
  '',
  '  // Log API: POST /api/log — print client log messages to terminal',
  '  if (pathname === "/api/log" && req.method === "POST") {',
  '    var logBody = "";',
  '    req.on("data", function(chunk) { logBody += chunk; });',
  '    req.on("end", function() {',
  '      try {',
  '        var parsed = JSON.parse(logBody);',
  '        var t = new Date().toISOString().substring(11, 19);',
  '        console.log("[" + t + "] [" + (parsed.tag || "Client") + "] " + parsed.message);',
  '      } catch(e) { /* ignore */ }',
  '      res.writeHead(204);',
  '      res.end();',
  '    });',
